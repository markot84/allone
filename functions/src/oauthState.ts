 ...
 *
 * Fail-safe: if the key is absent at runtime, state is left unsigned and
 * verifyState accepts unsigned — OAuth keeps working (no worse than before)
 * rather than breaking every connector login. With the key configured (the
