...
/** Number coercion for imported spreadsheet cells — MUST stay identical to
 * `parseLooseNumber` in `src/services/import.ts`, which is what the UI applies when the
 * same file is imported in the browser. Handles currency symbols, thin/nbsp spaces,
 * unicode minus, and both decimal conventions ("1.234,56" and "1,234.56"); a bare
 * `parseFloat` reads a Greek ERP export's "12.500,00" as 12.5.
 * Parity is locked by `__tests__/unit/importNumberParity.test.ts`. */
export function parseLooseNumber(value: string | number | null | undefined): number {
