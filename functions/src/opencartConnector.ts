...
export async function testOpenCartConnection(
  ...
  try {
    ...
    const res = await safeFetch(buildOpenCartRestUrl(storeUrl, 'rest/product_admin/products', { limit: '1' }), {
...
async function loginOpenCartAdminUser(
  ...
  const res = await safeFetch(buildOpenCartRestUrl(storeUrl, 'rest/admin_security/login'), {
    method: 'POST',
