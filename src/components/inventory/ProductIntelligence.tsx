  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        <span className="font-semibold text-[var(--orange-700)]">Φόρτωση δεδομένων προϊόντων…</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        {Array.from({ length: 5 }).map((_, i) => (
          <div
            key={i}
            className="rounded-xl border border-[var(--border)] bg-white p-4 h-[108px] shadow-sm"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {Array.from({ length: 3 }).map((_, i) => (
          <div key={i} className="h-[88px] rounded-xl border border-[var(--border)] bg-[var(--surface-2)] animate-pulse" />
        ))}
      </div>

      <Card padding="none" className="overflow-hidden border-[var(--border)] shadow-sm">
    ...
    return (
      <PageCanvas>
        <PageHeader
          ...
          description="Παρακολούθηση αποθέματος και απόδοσης προϊόντων"
        />
          ...
          {serverIntelligence.buildFailed ? (
            <>
              ...
              {serverIntelligence.buildError && (
                <p className="text-sm text-[var(--text-secondary)] mb-4">{serverIntelligence.buildError}</p>
              )}
  ...
  return (
    <PageCanvas>
      <PageHeader
        ...
        meta={
          hasServerAggregate ? (
            <div className="flex flex-wrap items-center gap-2 text-xs font-medium text-[var(--success-700)] sm:text-sm">
              <span>
        ...
        {inventoryAlerts.map((alert, index) => (
          ...
          >
            <Card
              ...
              className={`
                border-l-4
                ${alert.type === 'critical' ? 'border-l-[var(--danger-600)] bg-[var(--danger-light)]' :
                  alert.type === 'warning' ? 'border-l-[var(--orange-700)] bg-[var(--warning-light)]' :
                  'border-l-[var(--text-muted)] bg-[var(--surface-2)]'}
              `}
            >
              <div className="flex items-start gap-3">
                {alert.type === 'critical' ? (
                  <AlertCircle size={20} className="text-[var(--danger-600)] flex-shrink-0" />
                ) : alert.type === 'warning' ? (
                  <AlertTriangle size={20} className="text-[var(--orange-700)] flex-shrink-0" />
                ) : (
      ...
      <Card padding="none" data-product-table>
        <div className="px-4 pt-4 pb-3 border-b border-[var(--border)] flex flex-wrap items-end gap-3 bg-[var(--surface-2)]/60">
          ...
          <div className="flex flex-wrap gap-3 items-end">
            ...
            <label className="flex min-h-[38px] items-center gap-2 rounded-lg border border-[var(--border)] bg-white px-3 py-2 text-sm text-[var(--text-secondary)]">
              <input
                ...
                className="rounded border-[var(--navy-100)] text-[var(--nts-accent-text)] focus:ring-[var(--nts-accent)]/30"
              />
              <span className="whitespace-nowrap">Εμφάνιση no stock</span>
            </label>
            <label
              className="flex min-h-[38px] items-center gap-2 rounded-lg border border-[var(--border)] bg-white px-3 py-2 text-sm text-[var(--text-secondary)]"
            ...
            >
              <input
                ...
                className="rounded border-[var(--navy-100)] text-[var(--nts-accent)] focus:ring-[var(--nts-accent)]/30"
              />
...
function ProductRow({
  ...
  return (
    ...
    >
      ...
      <td className="px-3 py-2 hidden lg:table-cell">
        <span className="text-xs text-[var(--text-secondary)] truncate block max-w-[120px]">{product.brand || '—'}</span>
      </td>
      <td className="px-3 py-2">
        <Badge
          variant={
          ...
          {options.map(o => (
            <button
              ...
              className={`w-full text-left px-4 py-2 text-sm hover:bg-[var(--surface-2)] transition-colors ${
                o.value === value ? 'text-[var(--nts-accent-text)] font-medium bg-[var(--orange-50)]' : 'text-[var(--text-primary)]'
              }`}
            >
              {o.label}
            </button>
