...
export function Tooltip({ content, children, size = 14 }: TooltipProps) {
  ...
  useEffect(() => {
    ...
    if (arrow) {
      ...
      arrow.className = topPos < rect.top 
        ? 'tooltip-arrow absolute left-1/2 -translate-x-1/2 border-4 border-transparent border-t-[var(--navy-900)]'
        : 'tooltip-arrow absolute left-1/2 -translate-x-1/2 border-4 border-transparent border-b-[var(--navy-900)]';
    }
  }, [showTooltip]);
  ...
  return (
    ...
    >
      ...
      <HelpCircle 
        size={size} 
        className="text-[var(--text-muted)] hover:text-[var(--navy-500)] flex-shrink-0 cursor-help transition-colors" 
        aria-label="More information"
      />
      {showTooltip && (
        <div
          ref={tooltipRef}
          className="fixed z-[99999] px-3 py-2 bg-[var(--navy-900)] text-white text-xs rounded-lg shadow-lg pointer-events-none"
          style={{
            whiteSpace: 'pre-line',
            wordWrap: 'break-word',
            maxWidth: '400px',
        ...
        >
          {content}
          <div className="tooltip-arrow absolute left-1/2 -translate-x-1/2 border-4 border-transparent border-t-[var(--navy-900)]" />
        </div>
      )}
    </span>
  );
