...
export function ColumnExcelFilter({
  ...
  if (options.length === 0) {
    if (compact) return <span className="inline-block h-7 w-7 text-center text-xs text-[var(--navy-100)]">—</span>;
    return (
  ...
  const dropdown = open ? (
    <div className="absolute left-0 top-full z-[60] mt-1 flex max-h-72 min-w-[260px] w-max max-w-[min(100vw-2rem,320px)] flex-col rounded-lg border border-[var(--border)] bg-white shadow-lg">
      ...
      {selectionMode !== 'additive' && (
        ...
        <label className="flex cursor-pointer items-center gap-2 border-b border-[var(--border)] px-3 py-1.5 text-sm font-medium text-[var(--text-secondary)] hover:bg-[var(--surface-2)]">
          <input
            type="checkbox"
            ref={(el) => {
      ...
      <div className="max-h-52 overflow-y-auto p-1">
        {filteredOpts.map((o) => (
          <label
            key={o.id}
            className="flex cursor-pointer items-center gap-2 rounded px-2 py-1.5 text-sm text-[var(--text-secondary)] hover:bg-[var(--surface-2)]"
          >
            <input
              type="checkbox"
      ...
      <div className="flex items-center justify-between gap-2 border-t border-[var(--border)] bg-[var(--surface-2)]/90 p-2.5">
        <button
          type="button"
        ...
        <button
          type="button"
          className="whitespace-nowrap rounded-md btn-gold px-3 py-1.5 text-xs font-semibold text-white hover:btn-gold/90"
          onClick={applyDraft}
        >
          Εφαρμογή
        </button>
