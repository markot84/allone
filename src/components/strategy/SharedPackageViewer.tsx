    ...
    <div className="max-w-2xl mx-auto">
      ...
      <div className="flex items-start justify-between mb-6 pb-4 border-b-2 border-[var(--nts-accent)]">
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium bg-[var(--navy-500)] text-white hover:bg-[var(--navy-900)] transition-colors"
        >
          <FileDown size={14} />
          PDF
        </button>
      ...
      {/* Segments */}
      <div className="mb-6">
        <p className="text-[10px] font-semibold text-[var(--text-muted)] uppercase tracking-wider mb-2 pb-1 border-b border-[var(--surface-2)]">Segments</p>
        <div className="flex flex-wrap gap-1.5">
          {data.idealSegments.map(s => (
            <span key={s} className="px-2.5 py-1 rounded-full text-xs bg-[var(--success-light)] text-[var(--success-700)] border border-[var(--success-light)]">{s}</span>
          ))}
          {data.goodSegments.map(s => (
            <span key={s} className="px-2.5 py-1 rounded-full text-xs bg-[var(--surface-2)] text-[var(--text-secondary)] border border-dashed border-[var(--navy-100)]">{s}</span>
          ))}
        </div>
      </div>
      ...
      <div className="mb-6">
        ...
        <div className="space-y-1.5">
          {data.primaryChannels.map((ch, i) => (
            <div key={ch} className="flex justify-between items-center p-2.5 rounded-lg bg-[var(--surface-2)]">
