    ...
    <table className="min-w-full text-left text-sm">
      ...
      <tbody className="divide-y divide-[var(--border)]">
        {visibleRows.map((row, idx) => (
          ...
          <tr key={`${row.sku}__${row.changeDate}__${idx}`} className="hover:bg-[var(--surface-1)]">
            ...
            <td className="px-3 py-2 font-mono text-xs text-right whitespace-nowrap">
              ...
              {stockBySku && (() => {
                ...
                if (stock == null) {
                  return <p className="text-[10px] text-[var(--border-strong)]">απόθ. —</p>;
                }
      ...
      <tfoot className="border-t-2 border-[var(--border)] bg-[var(--surface-1)] text-xs font-semibold text-[var(--text-secondary)]">
        <tr>
...
function MarketingTable({ rows, limit }: { rows: MarketingSpendImpactRow[]; limit?: number }) {
  ...
  return (
    <table className="min-w-full text-left text-sm">
      ...
      <tbody className="divide-y divide-[var(--border)]">
        {visibleRows.map((row, idx) => (
          <tr key={`${row.id}__${idx}`} className="hover:bg-[var(--surface-1)] align-top">
