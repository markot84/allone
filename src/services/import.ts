...
const KNOWN_PROCUREMENT_HEADERS = new Set([
  ...
  'ΠΡΑΓΜΑΤΙΚΟΣ ΤΖΙΡΟΣ 12ΜΗΝΟΥ', 'ΠΡΑΓΜΑΤΙΚΟΣ ΤΖΙΡΟΣ 12 ΜΗΝΩΝ', 'ΤΖΙΡΟΣ 12ΜΗΝΟΥ',
...
export async function getLastImportDates(brandId: string | null | undefined): Promise<Record<string, Date>> {
  ...
  for (const job of normalized) {
    ...
    // Οτιδήποτε `partial` μετρά για «Τελευταίο sync»: Shopify/Woo κλπ. γράφουν partial αν διακοπεί ένα limb με imported=0.
    // `failed` επίσης (Entersoft HTTP κλπ.): να φαίνεται ότι έτρεξε προσπάθεια.
